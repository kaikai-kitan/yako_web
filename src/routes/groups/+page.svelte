	...
	async function createGroup() {
		...
		try {
			const res = await fetch('/api/groups/create', {
				...
				body: JSON.stringify({ name, description, startsAt: startsAt || null, endsAt: endsAt || null, joinPolicy })
			});
		...
		{#if canCreate}
			<section class="card">
				...
				{#if showForm}
					...
					<div class="field">
						<span class="field-label">参加方式</span>
						<div class="policy-toggle">
							<button type="button" class="policy-btn" class:on={joinPolicy === 'approval'} onclick={() => (joinPolicy = 'approval')}>
								<strong>承認制</strong><small>申請を管理者が承認して参加</small>
							</button>
							<button type="button" class="policy-btn" class:on={joinPolicy === 'open'} onclick={() => (joinPolicy = 'open')}>
								<strong>即参加</strong><small>コードを知っていれば即メンバー</small>
							</button>
						</div>
					</div>
					<p class="fine">終了日時を過ぎると参加受付は締め切られます（図の閲覧は可能）。承認制なら不正な参加を防げます。</p>
						...
						<li>
							<a class="group-item" href="{base}/groups/{g.id}">
								<div class="gi-main">
									...
									{#if g.my_role === 'owner'}<span class="gi-badge owner">主催</span>{:else if g.my_role === 'sub_admin'}<span class="gi-badge admin">運営</span>{/if}
									{#if g.my_status === 'pending'}<span class="gi-badge pending">承認待ち</span>{/if}
									{#if g.pending_count > 0}<span class="gi-badge review">承認待ち {g.pending_count}件</span>{/if}
									{#if isEnded(g.ends_at)}<span class="gi-badge ended">終了</span>{/if}
									{#if g.description}<span class="gi-desc">{g.description}</span>{/if}
									<span class="gi-meta">{fmtRange(g.starts_at, g.ends_at)}・{g.member_count}人・{g.join_policy === 'open' ? '即参加' : '承認制'}</span>
								</div>
...
<style>
	...
	.gi-badge.admin { background: rgba(95,122,82,0.16); color: #4a6a3a; }
	.gi-badge.pending { background: rgba(184,92,43,0.12); color: var(--accent-deep); }
	.gi-badge.review { background: var(--accent); color: #fff; }
	.gi-badge.ended { background: var(--surface-sunk); color: var(--ink-3); }

	.policy-toggle { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
	.policy-btn { display: flex; flex-direction: column; gap: 2px; text-align: left; padding: 10px 12px; border: 1.5px solid var(--line); border-radius: 10px; background: var(--surface); color: var(--ink-2); font-family: inherit; cursor: pointer; transition: border-color 0.15s, background 0.15s, color 0.15s; }
	.policy-btn:hover { border-color: var(--accent); }
	.policy-btn.on { border-color: var(--accent); background: var(--accent-tint); color: var(--ink); }
	.policy-btn strong { font-size: 0.85rem; color: var(--ink); }
	.policy-btn small { font-size: 0.68rem; color: var(--ink-2); line-height: 1.35; }
